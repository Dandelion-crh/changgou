...
public class SkuController {
    ...
    public Result descount(@RequestParam Map<String,Integer> dataMap){
        skuService.descount(dataMap);
        return new Result<>(true,StatusCode.OK,"扣减商品库存成功!");
    }
    ...
    public Result<List<Sku>> findByStatus(@PathVariable(value = "status") String status){
        ...
        return new Result<List<Sku>>(true,StatusCode.OK,"通过审核状态查询商品列表成功",skus);
    }
    ...
    public Result<PageInfo> findPage2(@RequestBody(required = false) List<Sku> skuList,@PathVariable("page") int page,@PathVariable("size") int size){
        PageInfo<Sku> page2 = skuService.findPage2(skuList, page, size);
        return new Result<PageInfo>(true,StatusCode.OK,"查询成功",page2);
    }
}
