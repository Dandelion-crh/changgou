...
public class SkuServiceImpl implements SkuService {
     ...
     */
    @Override
    public void descount(Map<String, Integer> dataMap) {
        for (Map.Entry<String, Integer> entry : dataMap.entrySet()) {
            String key = entry.getKey();
            Object num = entry.getValue();
            //将数据保存到数据库中
            int descount = skuMapper.descount(key, Integer.parseInt(num.toString()));
            if (descount <= 0){
     ...
     */
    @Override
    public PageInfo<Sku> findPage2(List<Sku> skuIdList, int page, int size) {
        // 静态分页
        PageHelper.startPage(page,size);
        // 搜索条件构建
        Example example = example(skuIdList);
        // 执行搜索
        return new PageInfo<Sku>(skuMapper.selectByExample(example));
    }
     ...
     * @return
     */
    public Example example(List<Sku> skuList){
